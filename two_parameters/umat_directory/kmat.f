...
********************************************
     ...
     + L,iphase,irradiate,implanted,C,stressvec,dstressinc,totstran,dtotstran,
...
C     *** INITIALIZE USER ARRAYS FROM STATE VARIABLES ***
      ...
      call kCRSS(iphase,tauc,nSys,G12,burgerv,gndtot,irradiate,implanted,
...
C     *** USE NEWTON METHOD TO DETERMINE STRESS INCREMENT ***
     ...
     +                gndall,gndcut,gndmob,implanted,dtime,nSys,ns,iphase,Lp,tmat,gammaDot)
...
C     *** DETERMINE DENSITY OF GNDs
    ...
    ! Edge-screw separated
     ...
     + nSys,irradiate,implanted,tauSolute,gammast,rhossd,iphase,rhofor,
