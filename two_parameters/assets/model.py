...
def runModel(umatFile, inputFile) -> str:
    ...
    try:
        ...
        if not wait_for_abaqus_completion(inputFile, timeout=8000):  # 8000s timeout
        ...
        if not wait_for_file_completion(odb_file, timeout=8000):  # 8000s timeout for file completion
